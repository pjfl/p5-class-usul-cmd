package Class::Usul::Cmd::Options;
...
Class::Usul::Cmd::Options - Command line processing

=head1 Synopsis

   use Class::Usul::Cmd::Types qw( Str );
   use Moo;
   ...
   use Class::Usul::Cmd::Options 'usage_conf' => {
...
sub import {
   ...
   my $apply_modifiers = sub {
      ...
      $with->('Class::Usul::Cmd::Trait::UntaintedGetopts');
   ...
   $info->{not_methods}{$option} = $option if $info;
...
sub _filter_attributes {
   ...
   my %filter_key = map { $_ => 1 } @option_attributes;
...
sub _validate_and_filter_options {
   ...
   $options{doc} = $options{documentation} unless defined $options{doc};
   $options{order} = 0 unless defined $options{order};

   if ($options{json}) {
      delete $options{autosplit};
      delete $options{negateable};
   ...
   $cmdline_options{format} .= '@'
      ...
      and ('@' ne substr $cmdline_options{format}, -1)
      and ('%' ne substr $cmdline_options{format}, -1);

   throw 'Negateable parameters are not usable with a non boolean values'
      if $cmdline_options{negateable} && defined $cmdline_options{format};
